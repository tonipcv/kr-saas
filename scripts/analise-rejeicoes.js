...
async function main() {
  try {
    ...
    // 5. Verificar notificações de rejeição enviadas
    console.log('\n5. Tentativa de verificar notificações de rejeição enviadas:');
    try {
      const notificacoesRejeicao = await prisma.$queryRaw`
        SELECT 
          n.id,
          n."userId",
          n.type,
        ...
        FROM 
          notifications n
        LEFT JOIN
          "User" u ON n."userId" = u.id
        WHERE 
          n.type LIKE '%REWARD_REJECTED%'
          OR n.title LIKE '%rejeitad%'
          OR n.title LIKE '%reject%'
        ORDER BY
          n."createdAt" DESC;
      `;
      console.log(formatResult(notificacoesRejeicao));
      console.log(`Total: ${notificacoesRejeicao.length} notificações de rejeição`);
    } catch (error) {
      console.log('Erro ao buscar notificações: tabela pode não existir ou ter outro nome');
      ...
      const resultado = await prisma.$transaction(async (tx) => {
        ...
        const resgateRejeitado = await tx.rewardRedemption.update({
          ...
          data: {
            status: 'REJECTED',
            rejectedAt: new Date(),
            rejectionReason: 'Teste de rejeição via script'
          }
        });
