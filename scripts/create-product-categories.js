...
async function main() {
  ...
  try {
    ...
    await runQuery(CREATE_CATEGORY_TABLE);
    ...
    const tableInfo = await getAllRows(db, "PRAGMA table_info(products)");
    ...
    if (!hasSubtitle) {
      ...
      await runQuery(db, "ALTER TABLE products ADD COLUMN subtitle TEXT");
    ...
    if (!hasCategoryId) {
      ...
      await runQuery(db, "ALTER TABLE products ADD COLUMN categoryId TEXT");
      console.log('Added categoryId column to products table.');
      
      // Create index on categoryId
      await runQuery(db, "CREATE INDEX IF NOT EXISTS products_category_id_idx ON products (categoryId)");
    ...
    const products = await getAllRows(db, `
