...
async function addClinicIdColumn() {
  try {
    ...
    const checkColumnExists = await prisma.$queryRawUnsafe(`
      ...
      WHERE table_name = 'referral_leads' 
      AND column_name = 'clinic_id'`);

    if (checkColumnExists.length === 0) {
      console.log('Adding clinic_id column to referral_leads table...');
      
      // Add the column
      await prisma.$executeRawUnsafe(
      `ALTER TABLE "referral_leads" ADD COLUMN IF NOT EXISTS "clinic_id" TEXT`
    );
      
      // Add foreign key constraint
      await prisma.$executeRawUnsafe(`
        ALTER TABLE "referral_leads" 
        ADD CONSTRAINT "referral_leads_clinic_id_fkey" 
      ...
      // Create index
      await prisma.$executeRawUnsafe(`
        CREATE INDEX IF NOT EXISTS "referral_leads_clinic_id_idx" 
        ON "referral_leads"("clinic_id")`);
...
async function backupReferralLeads() {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const backupTableName = `referral_leads_backup_${timestamp}`;
  ...
  try {
    // Create backup table
    await prisma.$executeRawUnsafe(
      `CREATE TABLE "${backupTableName}" AS SELECT * FROM "referral_leads"`
    );
...
async function migrateReferralLeadsData() {
  console.log('Starting referral leads data migration...');
  
  try {
    // Get all referral leads without a clinic_id
    const leadsToMigrate = await prisma.$queryRawUnsafe(`
      SELECT l.id, l."doctorId"
      FROM "referral_leads" l
      WHERE l."clinic_id" IS NULL
      AND l."doctorId" IS NOT NULL`);

    console.log(`Found ${leadsToMigrate.length} referral leads to migrate`);
    ...
    for (const lead of leadsToMigrate) {
      try {
        if (!lead.doctorId) {
          console.log(`Skipping lead ${lead.id} - no doctor associated`);
          results.skipped++;
          continue;
        }

        const primaryClinic = await findDoctorPrimaryClinic(lead.doctorId);

        if (!primaryClinic) {
          console.log(`No active clinic found for doctor ${lead.doctorId} (lead ${lead.id})`);
          results.failed++;
          results.errors.push({
            leadId: lead.id,
        ...
        // Update the lead with the clinic_id
        await prisma.$executeRawUnsafe(`
          UPDATE "referral_leads"
          SET "clinic_id" = '${primaryClinic.id}'
          WHERE id = '${lead.id}'`);

        console.log(`Successfully migrated lead ${lead.id} to clinic ${primaryClinic.id}`);
        results.success++;
      } catch (error) {
    ...
    console.log(`\nVerification: ${remainingUnmigrated[0].count} leads still without clinic_id`);
...
async function runMigration() {
  try {
    ...
    const backupTable = await backupReferralLeads();
    ...
    const results = await migrateReferralLeadsData();
    
    console.log('\nMigration process completed successfully!');
    console.log(`Backup table ${backupTable} has been preserved for safety.`);
    
    if (results.failed > 0) {
      console.log('\nWarning: Some leads failed to migrate. Please review the errors above.');
